...
def nParams(model):
    ...
    for p, parameter in enumerate(model.parameters()):
        if p in [0,2,4,6,8]:  # if layer is convolutional
...
for iter in range(lastIter, nIters):
    ...
    if iter > 0:
        ...
        if parameter == 'feature':
            ...
            newConfig['parameterRelated']['nFeatures']['slope'] += (4 * slopeUpDown)

        elif parameter == 'kernel':
            slopeUpDown = np.random.choice([1,-1]) # randomly increase or decrease slope
            newConfig['parameterRelated']['kernelSize']['slope'] += (.5 * slopeUpDown)

        # calculate new set of params
        middleLayer = int(nLayers / 2)
        for feature in newConfig['parameterRelated']:
            fmean = newConfig['parameterRelated'][feature]['mean']
            fslope = newConfig['parameterRelated'][feature]['slope']
            newConfig['parameterRelated'][feature]['values'] = [int(fmean + (l - middleLayer) * fslope) for l in range(nLayers)]
    ...
    if newConfig not in results['allConfigs']:
        ...
        model = CogNet(nLayers,
                       newConfig['parameterRelated']['nFeatures']['values'],
                       newConfig['parameterRelated']['kernelSize']['values'],
                       newConfig['parameterOrthogonal']['stride'],
                       nCategories)
        ...
        modelCommit = ModelCommitment(identifier=identifier, activations_model=activations_model,layers=['features.12'])
