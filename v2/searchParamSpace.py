...
for iter in range(lastIter, nIters):
    ...
    if iter > 0:
        ...
        if parameter == 'feature':
            ...
            newConfig['nFeatures']['slope'] += (4 * slopeUpDown)

        elif parameter == 'kernel':
            slopeUpDown = np.random.choice([1,-1]) # randomly increase or decrease slope
            newConfig['kernelSize']['slope'] += (.5 * slopeUpDown)
    ...
    if newConfig not in results['allConfigs']:
        ...
        modelCommit = ModelCommitment(identifier=identifier, activations_model=activations_model,layers=['features.17'])
